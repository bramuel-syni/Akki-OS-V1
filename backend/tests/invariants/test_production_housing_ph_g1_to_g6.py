...
async def test_ph_g3_readyz_returns_200_with_parity_31_when_healthy():
    ...
    if resp.status_code == 200:
        ...
        assert body["parity_count"] == 33
        assert body["expected_parity"] == 33
        assert body["db"] == "ok"
    else:
...
async def test_ph_g5_build_info_returns_git_sha_and_parity():
    ...
    assert body["parity_count"] == 33  # Same counter as PH-E3.
...
def test_ph_g6_shared_counter_returns_31():
    ...
    assert count_frozen_contract_snapshots() == 33
    assert EXPECTED_PARITY == 33
