...
**Last update:** 2026-07-06 (Commercial cut executed per BCR v1.4 §12 — Phase 7 B-2 buyer wizard variant + Phase 6 commercial half + Commercial Reference Application UI cut with salvage-preservation; parity 26 held; MAN-G1/G2/G3 named gates GREEN. Owner Step-2 dispatch complete, awaiting acceptance + push confirmation.)

> Any duration / credit / turn number referenced below is a **Provisional planning anchor — not a commitment. Relative weight only.**

## Live State
- **Current gate:** **COMMERCIAL CUT CLOSED 2026-07-06 — Subtractive change per BCR v1.4 §12 with mandatory preservation. Salvage tree at `/app/salvage/commercial_cut_2026_07_06/`.** Cut execution: Phase 7 B-2 **buyer wizard variant** (state machine + router + Sonnet driver + buyer helpers + tests + §5.1/§5.2/§5.3 pages + Playwright smoke + Jest binding-copy) → CUT. **Phase 6 commercial half** (dual-delta commercial coding + SonnetWizardAgent) → CUT. `QuoteEnvelope_v0` + `pricing_tiers.vN.json` + `AsyncDeliveryAccepted_v1.quote` field + `WizardCommitState_v0.variant="buyer"` Literal value → ORPHAN-IN-PLACE (byte-identical). Pull-sample-for-purchase → VERIFIED-ABSENT. Operator §2 / Engineer §4 / Master-Admin §6 / Ask Console `/` / Shared §8 barrel / Legacy — VERIFIED-UNTOUCHED. Parity 26/26 byte-identical.
