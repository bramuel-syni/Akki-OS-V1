...
## Live State
- **Current gate:** **PHASE 4b CLOSED** (2026-07-04) — §6.2 composed-conclusion path landed live at v2 dispatch (warm+composed_conclusion → HTTP 200 with `ComposedConclusion_v0` 18th frozen contract; below-floor at conclusion class → HTTP 422 via `Service1Refusal_v0(reason=composition_below_floor)` per §6.2.6). ComposedConclusion@v0 snapshot SHA `a85eaf95...`. Wire Union widened per Stage A settled table: `Union[DispatchResult @501, AdmissionRefusal_v0 @422, ComposedConclusion_v0 @200, Service1Refusal_v0 @422, <§6.1 payload dict> @200]`. Ruling 3 wire-shape gate VERIFIED GREEN post-widening (fold-in verification replacing parked three-artifact verification per Owner's proportionate call). Condition B1 LOAD-BEARING at gate 13: composed_conclusion.py threads `conclusion_class` from Solva; no local recomputation (AST + grep-negative verified). Phase 4a Stage B CLOSED (remediation applied 2026-07-04). Phase 3 CLOSED. Phase 2 CLOSED. Substrate-Drop v2 CLOSED. Phase 0 CLOSED. Phase 1 CLOSED. G5b CLOSED. Backend surface FROZEN except additive-only Phase 4b landing (18th frozen contract + new service module + dispatch integration + router Union widening; no v0 mutations; no v0 SHA drift on 7 protected files; all 17 prior frozen contract sources byte-identical; no Service1Refusal mutation; no AdmissionRefusal_v0 mutation; no ObjectiveRequest_v2 mutation). Close report on-disk canonical: `/app/docs/close_reports/phase_4b.md` SHA-256 `2781313b28e152277e41f135801e4f5e3f0a3b083aa50a53ef3ab634c9cfb6c7`.
...
- **Last green CI:** 446/446 backend at 2026-07-04 (+ 18/18 frontend gate tests unchanged from G5b). Substrate-drop gate 9/9 green. Mechanical parity invariant 3/3 green at 18 entries.
...
- **Frozen contracts:** 18 (was 17 at Phase 4a close; +1 `ComposedConclusion_v0` at Phase 4b, snapshot SHA `a85eaf95...`). All 18 have canonical `.contract_snapshot.json` files; mechanical parity invariant enforces bijection.
