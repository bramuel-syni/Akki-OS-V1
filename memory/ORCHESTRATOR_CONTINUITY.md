...
- Current gate: **A2 CLOSED (2026-07-02T02:15Z)**. `Service1Refusal@v0` shipped as 14th frozen contract; `POST /api/service_1/run` now returns flat JSONResponse-based governed refusal envelope (`outcome="refused"`) at HTTP 422 across three reason codes (`no_defensibility_floor`, `no_lawful_basis`, `composition_below_floor`); new raise site triggered by `len(eligible) == 0` reads Ring-5-governed per-unit classes (D6a) and reduces via `max` (D7a). `backend_contract_surface_v1.md` §1/§2/§4/§5.3/§5.4/§8 realigned; signature bumped to `-v1.1-a2-e1_dev-20260702T021500Z`. HANDOFF-DOWNLOAD ROUTE (`GET /api/handoff/backend_contract_surface_v1`) shipped earlier in the session, still live. Awaiting user go on G5b dispatch. All prior phases still CLOSED.
- Awaiting: user judgment on G5b dispatch (Frontend Operator Console + Consumer Terminal) OR real RMS material for G2b OR governance decisions to unlock closed seams.
- Last green CI: **355/355** at **2026-07-02T02:15Z**. Command: `cd /app && make ci`. Delta from G6: +15 tests (+7 handoff route + +7 A2 refusal envelope + +1 A2 snapshot invariant).
...
- New frozen contracts (addition at A2): `Service1Refusal@v0`. Total frozen contracts: **14**.
- A2 acceptance-gate items (all green): (i) STEP 0 preconditions clean (§204/§247 wording check confirmed content-categories-only, HAZARD-STOP (f) did NOT fire); (ii) 14th frozen contract shipped with snapshot + snapshot invariant passing; (iii) `objective_text` added to `Service1RunRequest`; (iv) service layer plumbed at 3 refusal sites + `_max_supported_class` helper reads Ring-5 governed values + max-not-min semantic verified; (v) router change shipped (JSONResponse, `responses={200, 422}` OpenAPI); (vi) 7 invariant tests passing; (vii) `workflow_dispatch` added to `g0-gate.yml`; (viii) handoff artifact realigned across §1/§2/§4/§5.3/§5.4/§8; (ix) lift_manifest + BUILD_JOURNAL + this file + PHASE_STATE all updated.
- **Closed seams (four)** — unchanged from G6:
...
- Discipline observation carried forward (X1): `pipeline.py:75-76` in solva_depth performs a redundant `conclusion_class(lb)` recompute after `enforce()` already threaded `computed_class` on the Refusal dataclass. Not a doctrine violation (same-input deterministic call), tracked separately per user directive; do NOT fix in A2.
