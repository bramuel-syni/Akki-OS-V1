...
- **Latest gate:** **Phase 8 Stage B-5a CLOSED 2026-07-06** — Compliance Console read/prove (v2.1 §4.1-4.3) LIVE with Amendments 1 + 2. 3 new frontend pages (`/compliance/{,prove/:traceId,retention}`), 2 new backend endpoints (`GET /api/compliance/{retention_config,refusals}`) with `dpo`|`admin` gate via existing E2 4-code registry. Trace endpoint `GET /api/northena/trace/{trace_id}` carries Amendment 1 auth-branch (allowlist projection for anonymous/lesser roles → 4 fields byte-equivalent to trust-receipt spec; full record for `dpo`/`master_admin`/`admin`). Backend CI **847/847** (from 814; +33). Jest **92/92** (from 70; +22). Playwright chromium **26/26** (from 20; +6). Parity 26/26 byte-identical. Amendment 1 gates 2/2 GREEN. Amendment 2 gates 5/5 GREEN. **Refusals-by-Month Coverage Statement** at close report §V: 2 families LEDGERED ✅ (admission-sync + outer-gate V2) + 3 families UN-LEDGERED ❌ FINDINGS filed as new §0.2 debt (resolution surface at Seam 3). DPO wizard_transcript §0.2 debt RESOLVED. **Sequence amendment (Owner-ratified, binding):** B-5a → Seam 3 + §8 checker → B-5b — supersedes BCR v1.4 §3.5 "before B-5" posture. Rule 2: actual ~2805L vs restated band [2240, 2640] → +6.3% ABOVE → SPLIT-DISPOSED (mandate-forced portion ratified; orchestrator-side backend-impl under-estimate is standing-correction candidate for Seam 3 sizing). Close report: `/app/docs/close_reports/phase_8_b_5a.md` SHA `c48672b4562f330129396ef3b90aaffdfb45f6c2714a671e4aaee2953a7c8baf`.
- **Previous gate:** Phase 8 Conformance Map CLOSED 2026-07-06 (SHA `e747a0f6...`; manifest preamble `46d81b8b...` + `59d43a95...`).
- **Awaiting:** Owner ratification of B-5a close. On acceptance → **Seam 3 + §8 checker** dispatch (authorized deletion path + cross-console consequence-class checker; closes new §0.2 debt on un-ledgered refusal families).
- **Sequenced work (post-Seam 3):** B-5b (Compliance Console rulebook writes §4.4-4.5) → Phase 9 Extraction Console sampling → Transform forms (post-B-5) → §5.4 Dual-actor Integration Console.
