...
#### §7.1.γ.1 — Carrier + wire-shape gate (Owner refinement, 2026-07-06)

**Owner refinement verbatim:** *"Sidecar is the right container, wrong as a loose Dict. Pin the key. The refusal-family value is a queried, grouped dimension (it feeds the refusals-by-month aggregate and the regulator surface), not audit metadata — burying it in an unschematized stamp_audit Dict means every aggregate reads a key with no guarantee it exists or holds a registry-valid value. But a top-level typed field mutates NorthenaLedgerRow_v1 and breaks byte-identity. Resolve both the same way §6.1 was: keep the sidecar, pin `stamp_audit['refusal_family']` under a load-bearing wire-shape gate asserting presence and registry-validity on every refusal-terminal row. Contract stays byte-identical, parity 26 holds, the aggregate reads a guaranteed key."*

**Carrier mechanism (binding):** the registry-backed constrained-str family value lives at **`stamp_audit["refusal_family"]`** — a pinned key inside the existing `stamp_audit: Optional[Dict]` sidecar on `NorthenaLedgerRow_v1`. Contract stays byte-identical (parity 26 holds). Every emission of a refusal-terminal row via `services/compliance/refusal_ledger.py::emit_refusal_ledger_row` sets this key with a registry-valid value from `refusal_families.v0.json`. Aggregate reads (`services/compliance/refusals_aggregate.py`) and the regulator surface (v2.1 §4.1 Refusals card + §4.2 Prove-one-run trace view) both consume this pinned key.

**Load-bearing wire-shape gate (binding):** **`test_refusal_terminal_row_carries_registry_valid_refusal_family_in_stamp_audit`** — LB. Asserts on **every** refusal-terminal row (any `NorthenaLedgerRow_v1` with `decision="refused"`): (a) `stamp_audit` is present (non-null); (b) `stamp_audit["refusal_family"]` is present as a key; (c) its value is a registry-valid entry per the currently-loaded `refusal_families.vN.json`. **Retirement condition: never** — this is a permanent load-bearing gate protecting aggregate + regulator-surface consumers from unschematized reads. Applies across all refusal-terminal rows (sync + async emission paths; existing + newly-instrumented emission sites).

**Precedent cited:** §6.1 Sub-stage 3 (in this same proposal, current amended text) resolves the same class of problem for CK-B1 dual-identity via the sidecar+pinned-keys pattern — `services/checker/countersign_ledger.py` (proposal line 319) emits with `stamp_audit: {data_class, rule_class, consequence_class, initiator, checker, initiated_at, countersigned_at}` (pinned keys, not a loose Dict) + gate **CK-G2 `test_countersign_row_carries_both_identities`** (proposal §6.1 test matrix line 342) asserts LB presence of the pinned identity keys on every countersign row. The Owner refinement resolves Sub-stage 1's refusal-family carrier by **the same pattern** — sidecar container, pinned key names, LB wire-shape gate asserting presence + registry-validity on every terminal row of the relevant class.
