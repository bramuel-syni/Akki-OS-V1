# Outstanding Work & Gap Register · v1.5 · 2026-07-15
...
## §15 · Amendment 5 · 2026-07-15 (Owner-ratified) — G-3 Operating Values v1.1 close

Predecessor: v1.4 at SHA `1e67daaba99e3319a80ed30ee09dc42221dc734b1c0cc40d94e0d7e7a70f1172`. Ruling: `docs/rulings/g3_operating_values_v1_1_2026-07-15.md`.

G-3 Operating Values v1.1 closed. Eight deltas landed (six from Stage A + two from scope-4 addition):

- **Delta 1 · §4 G-3 line status:** `[QUEUED · Stage A drafted 2026-07-15]` → `[EXECUTED · atomic-commit 2026-07-15 · close_reports/g3_operating_values_v1_1.md]`.
- **Delta 2 · §5 Operating Values pin:** `v1.0` → **`v1.1 sibling` in force alongside v1.0** (`docs/requirements/operating_values_v1_1.md` · SHA `3a3cff3be0cb59d28cd06a7e25123155d6984323f78e386687ee05c20f2d9c5b`; v1.0 preserved byte-identical per Standing Rule v3).
- **Delta 3 · §5 Sixth seam value (F2):** `[EAB-scoped]` → `[LANDED in Op. Values v1.1 §6.6 + backend/services/multi_instance/onboard_context.py::SeamValues.quarantine_systemic_halt_threshold]` (2% DEFAULT · per-instance · S2.onboard-set per MC-E3 α · dual-control on change per §6).
- **Delta 4 · §5 spaCy NER conformance row:** `[RECONNECTED · IF-1]` → `[RECORDED in Op. Values v1.1 §1 Rung 2 · fail-closed de-id role · live in Shield]`.
- **Delta 5 · §5 Diarization row:** `[split-target]` → `[BUILT (Silero VAD · FACT · live) + DECIDED-TARGET (pyannote · NORM · license-verify-at-acquisition FLAG · NeMo Apache-2.0 fallback)]`.
- **Delta 6 · §5 TQ v1.0 (new):** landed as sibling requirements canon; artifact at `docs/requirements/transformation_quality_spec_v1.md` SHA `78af90cf64409364c9b9d97bbc2b7f0507f6b824ee1bc41bd610a79f1a44967e`. Absorbed by citation into Op. Values v1.1 quality section (TQ §5.1 speech values + TQ §6 MOAC M-a..M-f; zero verbatim duplication).
- **Delta 7 · §5 Critic Seam v1.1 (new):** sibling landed at `docs/requirements/critic_seam_spec_v1_1.md` SHA `ad4529b9462cf789ab577f7f8a4ba4ed1fe8f33a096fce6a70669a826b8c5605` · Part B pointer to TQ §7. v1.0 preserved byte-identical.
- **Delta 8 · §17 sequencing anchor:** G-3 → EAB-1 → EAB-2 (Parity 31→32 seal event at its Tier-1 relay) → EAB-3, with Critic Seam Tier-1 cells riding their closes; critic-pass phase slots after EAB-1 at Owner sequencing judgment. **Unchanged** (documented for provenance).

**Consolidation posture (G3-E1 α):** SeamValues extends to 6 fields in-place · no new contract version · no Parity 31 contact · existing `instance_1` backfilled via `initial_set`-marked ledger row per MC-E3 α semantics · onboard writes 8 initial-set rows total per landing.

**Alias canonicalization discipline (from G-2 close, standing):** short-form canonicalization preserved (`S3.prove` · `S4.verify`); legacy long-form aliases remain retired.

**6-row R4 sidecar minted** at `docs/registry/function_promise_registry_v1_g3_sidecar.md` (all attach to existing v1 promises · zero new promises · 4 × `PROM-S1-frozen-wire-contract` + 2 × `PROM-S3-audit-trail-immutable`).

**D-10 STANDING PRACTICE (Owner-ratified at G-2 close):** every Stage-A / close-report terminal reply carries D-1..D-11 self-audit table. G-3 close reply honors.

**Scope-4 4d disclosure:** de-risking spec absent as standalone on-disk artifact (embedded in Registry Doctrine v1.0 §Part VII); per Owner HALT trigger, no touch attempted. TQ §11 line 174 already carries the re-scope-by-reference clause within TQ itself.

Register status updates:
- §4 · G-3 row now `[EXECUTED · atomic-commit 2026-07-15]`.
- §5 · Op. Values pin flipped to v1.1 sibling.
- §5 · TQ v1.0 line added (canon).
- §5 · Critic Seam v1.1 line added (sibling).
- §5 · Sixth seam value line added (LANDED).
...
*End of register v1.5. Standing Rule v3 · on-disk canonical.*
